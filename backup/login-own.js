...
function validateForm(event) {
    ...
    if (userName.value === "admin" && passWordVal === "Admin@123") {
        window.location.href = "http://localhost/hr/startbootstrap-sb-admin-2-gh-pages/allEmployees.html";
    }
    else {
        fd = new FormData();
...
function fetchDashBoard(user) {
    ...
    // fetch("payrollBreakdown.php?user="+user)
    // .then(function(response)
    // {
    //     return response.json();
    // })
...
function writeToDashBoard() {
    ...
    console.log("ct", data.complaint_id);
    console.log(typeof (data.complaint_id));
    if (typeof data.complaint_id == 'undefined' || typeof data.complaint_id == 'NULL') {
        ...
        //DO INNER HTML PAH
        complainttitle.innerHTML = data.type;
        complaintstart.innerHTML = data.issue_date;
        complaintend.innerHTML = data.close_date;
    }
...
function generateReport(event) {
    ...
    */
    ...
    doc.text(20, 120, "email_id id: " + data.email_id);
...
function openComplaint() {
    var popup = window.open("http://localhost/hr/startbootstrap-sb-admin-2-gh-pages/createComplaint.html", "popup", "width=400,height=500");
}

function createPieChart() {
    console.log("in pie fn");
    ...
    var myPieChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                data: [data.basic_salary, data.tax, data.overtime_pay, data.bonus],
